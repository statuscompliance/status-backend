...
describe('storeGuaranteePoints', () => {
  ...
  const agreement = {
    terms: {
      guarantees: [{ id: 'g1', of: [{ window: { period: 'monthly' } }] }],
    },
  };
  ...
  it('should detect a duplicate point and add an error message', async () => {
    ...
    vi.spyOn(models.Point, 'findOne').mockResolvedValue({
      id: 'duplicatedPoint',
    });

    guaranteeStates.values = false;
    guaranteeStates.evidences = [{ computationGroup: 'group1' }];
  ...
  it('should catch and return an error when bulkCreate fails', async () => {
    ...
    vi.spyOn(models.Point, 'bulkCreate').mockRejectedValue(
  ...
  it('should skip guaranteeStates if guaranteeTerm.of[0] does not have window', async () => {
    agreement.terms.guarantees[0].of = [{}]; // without window
