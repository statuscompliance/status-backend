...
export async function executeEndpointFlow(endpointUrl, msg) {
  ...
  try {
    ...
    if (response.status === 200){
      logger.debug('Succesfully executed flow with status:', response.status);
    }else{
      logger.debug('Unexpected error while executing flow:', response.status);
    }
    return response;
...
export async function getAllFlows() {
  try {
    ...
    if (response.status === 200) {
      logger.debug('Succesfully retrieved all flows:', response.data)
    }else{
      logger.debug('Unexpected error while retrieving all flows:', response.status);
    }
    return response;
