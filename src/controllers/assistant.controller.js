...
export async function createAssistant(req, res) {
    try {
        ...
        res.status(201).json({
            message: `Assistant ${name} with id ${response.assistantId} created successfully`,
        });
    } catch (error) {
        res.status(500).json({
            message: `Failed to create assistant, error: ${error.message}`,
        });
    }
}
...
export async function getAssistants(req, res) {
    try {
        ...
        res.status(500).json({
            message: `Failed to get assistants, error: ${error.message}`,
        });
    }
}
...
export async function deleteAllAssistants(req, res) {
    try {
        ...
        res.status(500).json({
            message: `Failed to delete all assistants, error: ${error.message}`,
        });
    }
}
...
export async function getAssistantInstructions(req, res) {
    try {
        ...
        res.status(500).json({
            message: `Failed to get assistant instructions, error: ${error.message}`,
        });
    }
}
    ...
    try {
        ...
        try {
            await openai.beta.assistants.update(
                updatedAssistant.dataValues.assistantId,
                {
                    instructions: instructions,
                }
            );
        ...
        res.status(500).json({
            message: `Failed to update assistant instructions, error: ${error.message}`,
        });
    }
}
...
export async function deleteAssistantById(req, res) {
    try {
        ...
        res.status(500).json({
            message: `Failed to delete the assistant, error: ${error.message}`,
        });
    }
}
